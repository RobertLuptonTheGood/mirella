...
char *init_malloc P_ARGS(( normal needed ));
...
char *memalloc P_ARGS(( char *name, normal size ));
...
char *tryalloc P_ARGS(( normal nbytes ));
...
double farsum P_ARGS(( float *buf, normal n ));
double fasolve P_ARGS(( float *buf, normal n, double value ));
double faverage P_ARGS(( float *buf, normal n ));
...
double naverage P_ARGS(( normal *buf, normal n ));
...
double saverage P_ARGS(( short *buf, normal n ));
...
Void *m_sbrk P_ARGS(( normal size ));
...
int number P_ARGS(( char *str, normal *n ));
...
int rexpect P_ARGS(( char *addr, normal count ));
int rget1char P_ARGS(( void ));
normal round P_ARGS(( double f ));
...
normal mseek P_ARGS(( normal pos, int org ));
...
void crcchk P_ARGS(( normal *ibuf, normal nibuf, normal *pcksum, normal *pcrc ));
...
void faradd P_ARGS(( float *src, float *dest, normal n ));
...
void gd_todis P_ARGS(( Void *from, Void *to, normal nby ));
void gend P_ARGS(( void ));
void gers P_ARGS(( void ));
void get_dic P_ARGS(( char *filename ));
int image_to_buf P_ARGS(( normal *x, normal *y));
...
void naradd P_ARGS(( normal *src, normal *dest, normal n ));
...
void saradd P_ARGS(( short *src, short *dest, normal n ));
...
void waitfor P_ARGS(( normal t ));
void waituntil P_ARGS(( normal next, normal tp ));
